...
const sparklinePalette = ['#17624a', '#0f766e', '#ce7b34', '#7c3aed'];
const sparklineData = [
  [44, 51, 58, 62, 66, 70],
  [30, 33, 37, 38, 41, 45],
  [22, 35, 40, 46, 52, 58],
...
const DashboardView: React.FC<Props> = ({ year, data }) => {
  ...
  const publicationTrend = [
    { year: '2025', target: 590, actual: 590 },
    { year: '2026', target: 620, actual: 485 },
  ];
  ...
  return (
    <div className="space-y-5 lg:space-y-6">
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {summary.map((s, i) => (
          <article key={s.title} className="surface-card metric-card stagger-in rounded-2xl p-5">
            ...
            <h3 className="display-font text-3xl font-bold text-[var(--ink)]">{s.value}</h3>
            <div className="mt-4 flex items-end gap-1.5">
              {sparklineData[i].map((v, idx) => (
                <span
                  key={`${s.title}-${idx}`}
                  className="h-8 flex-1 rounded-sm"
                  style={{
                    height: `${16 + Math.max(7, (v / Math.max(...sparklineData[i])) * 22)}px`,
                    backgroundColor: sparklinePalette[i]
                  }}
                />
              ))}
            </div>
      ...
      <section className="grid grid-cols-1 gap-5 xl:grid-cols-2">
        ...
        <article className="surface-card panel-in rounded-3xl p-5 sm:p-6">
          <div className="mb-5 flex items-center justify-between gap-2">
            <h3 className="display-font text-lg font-bold text-[var(--ink)]">Tren IKU 6: Target vs Capaian</h3>
            <span className="rounded-full bg-[var(--accent-soft)] px-2.5 py-1 text-xs font-semibold text-[#9d5f2d]">Publikasi</span>
          </div>
          <div className="h-[320px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={publicationTrend}>
                <defs>
                  <linearGradient id="targetGradient" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#17624a" stopOpacity={0.2} />
                    <stop offset="95%" stopColor="#17624a" stopOpacity={0} />
                  </linearGradient>
                ...
                <YAxis tick={{ fontSize: 12, fill: '#63756b' }} axisLine={false} tickLine={false} />
