...
const IkuManagementView: React.FC<Props> = ({ data, onDataChanged, source }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!canMutate) {
      setError('Backend MySQL belum aktif. Jalankan API untuk CRUD.');
      return;
    }
  ...
  const handleImport = async (event: React.ChangeEvent<HTMLInputElement>) => {
    if (!canMutate) {
      setError('Backend MySQL belum aktif. Jalankan API untuk import Excel.');
      return;
    }
    ...
    <div className="space-y-5">
      <section className="surface-card rounded-3xl p-5 sm:p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className={`rounded-full px-3 py-1 text-xs font-semibold ${canMutate ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
            {canMutate ? 'Data source: MySQL API' : 'Data source: Local fallback'}
          </div>
        </div>
